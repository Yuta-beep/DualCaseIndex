...
## ファイル
- `prep_casefilter.c` : 索引構築用統合ファイル（types.h + index.h + build.c + main）
- `search_casefilter.c`: 探索用統合ファイル（types.h + index.h + build.c(deserialize) + search.c + main）
- `src/` : 元のソースコード（参照用・分割バージョン）
